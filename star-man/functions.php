    ...
    'robot_function_ontargetchange' => function($objects){
        ...
        if (true) {
            switch ($target_robot->robot_token){
                case 'proto-man': {
                    $this_robot->set_quote('battle_victory', 'Dark Man, you\’ve gotten sloppy! The real Proto Man would never be caught with a gaudy scarf like that!');
                    break;
                }
                case 'toad-man': {
                    $this_robot->set_quote('battle_victory', 'Eugh, so many shades of green! I’d never be caught dead wearing anything that gross!');
                    break;
                }
            }
        }
