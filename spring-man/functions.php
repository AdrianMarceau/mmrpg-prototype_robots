    ...
    'robot_function_ontargetchange' => function($objects){
        ...
        if (true) {
            switch ($target_robot->robot_token){
                case 'bounce-man': {
                    $this_robot->set_quote('battle_victory', 'Your Boxercise program really works wonders! I just wish the ceilings at Boing-Boing park weren\’t so low.');
                    break;
                }
                case 'napalm-man': {
                    $this_robot->set_quote('battle_victory', 'You still sore about losing the Battle and Chase?');
                    break;
                }
            }
        }
